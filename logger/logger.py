...
class Log:
    ...
    def display(self, file_name: str, function_name: str, reason: str, data: str):
        if self.is_display and self.mode == "debug":
            print(f"[ {self.domain}.{file_name}.{function_name} ] ", end="")
            print(f"> {reason}\n> {data}")

    def save(self, file_name: str, function_name: str, reason: str, data: str):
        if self.is_save:
            ...
            formatted = time.strftime("%Y_%m_%d-%H_%M_%S", local_time)
            increase_number = len(os.listdir(self.save_path)) + 1
            with open(f"{self.save_path}/{formatted}-{increase_number}-{self.domain}.{file_name}.{function_name}", mode="w", encoding="utf-8") as f:
